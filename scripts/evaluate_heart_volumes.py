...
def main():
    ...
    output_dir = os.path.join(PROJECT_ROOT, "results", "predictions", "volume_eval")
    ...
    print("=" * 50)
    print("Evaluate Whole-volume Heart MRI Cases")
    print("=" * 50)
    print("Device:", device)
    ...
    for idx, case_name in enumerate(val_cases, start=1):
        ...
        print(
            f"{case_name} | "
            f"Raw Dice: {result['raw_dice']:.4f}, Raw IoU: {result['raw_iou']:.4f} | "
            f"LCC Dice: {result['lcc_dice']:.4f}, LCC IoU: {result['lcc_iou']:.4f}"
        )

    df = pd.DataFrame(results)

    csv_path = os.path.join(PROJECT_ROOT, "results", "heart_volume_metrics.csv")
    ...
    print("Summary")
    print("=" * 50)
    print(df[["case", "raw_dice", "raw_iou", "lcc_dice", "lcc_iou", "raw_pred_voxels", "lcc_pred_voxels", "gt_voxels"]])
